...
class CacheService:
    ...
    def _get_file_path(self, key, timestamp=None):
        ts = timestamp or datetime.now().strftime("%Y%m%d")
        return os.path.join(self.cache_dir, f"{key}_{ts}.json")

    def get(self, key):
        """Lấy dữ liệu từ cache nếu còn hạn"""
        # Tìm tất cả các file có pattern: key_*.json
        search_pattern = os.path.join(self.cache_dir, f"{key}_*.json")
        for file_path in glob.glob(search_pattern):
            filename = os.path.basename(file_path)
            # Trích xuất timestamp (giả định định dạng key_YYYYMMDD.json)
            date_str = filename.replace(f"{key}_", "").replace(".json", "")
